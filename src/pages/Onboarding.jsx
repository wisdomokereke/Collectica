  ...
  freelancer: [
    {
      ...
      stats: [{ value: '0%', label: 'Payment disputes' }, { value: '100%', label: 'Contract protection' }],
    },
    {
      emoji: '🔐', tag: 'Your Identity',
      title: 'One account.\nYour whole career.',
      ...
      stats: [{ value: 'Real', label: 'Verified reviews' }, { value: 'Yours', label: 'Permanent record' }],
    },
    {
      emoji: '🎯', tag: 'Your Skills',
      title: 'Be found by\nthe right clients.',
      body: 'Clients search by skill. The more specific you are, the better the match. Pick what you\'re best at.',
      stats: [{ value: '5', label: 'Skills max' }, { value: 'Smart', label: 'Matching' }],
    },
    {
      emoji: '💰', tag: 'Escrow Wallet',
      title: 'Your money.\nSafe and instant.',
      body: 'When a milestone is approved, payment lands in your Collectica wallet immediately. Withdraw to your bank anytime.',
      stats: [{ value: 'Instant', label: 'On approval' }, { value: 'Anytime', label: 'Withdrawal' }],
    },
    {
      emoji: '✍️', tag: 'Signing Password',
      title: 'Your signature.\nYour protection.',
      body: 'Your signing password is how you authorise every contract. Simple, secure, and only you know it.',
      stats: [{ value: 'Unique', label: 'To you only' }, { value: 'Locked', label: 'Once signed' }],
    },
  ],
  client: [
    {
      emoji: '💼', tag: 'For Clients',
      title: 'Hire with\nconfidence.',
      ...
      stats: [{ value: '100%', label: 'Scam protection' }, { value: '0', label: 'Upfront risk' }],
    },
    {
      emoji: '🔐', tag: 'Your Identity',
      title: 'One account.\nEvery project.',
      body: 'Your Collectica profile holds every contract you\'ve ever created. Freelancers can see your track record too.',
      stats: [{ value: 'Real', label: 'Verified profile' }, { value: 'Clear', label: 'Project history' }],
    },
    {
      emoji: '📋', tag: 'Your Business',
      title: 'Post jobs.\nHire the best.',
      body: 'Tell freelancers what you need. The clearer your profile, the better the talent that comes to you.',
      stats: [{ value: 'Fast', label: 'Talent match' }, { value: 'Verified', label: 'Freelancers only' }],
    },
    {
      emoji: '🏦', tag: 'Escrow Wallet',
      title: 'Fund once.\nPay in stages.',
      body: 'Deposit into your Collectica wallet. Lock funds per contract. Release per milestone as work gets done.',
      stats: [{ value: 'Staged', label: 'Payments' }, { value: 'Full', label: 'Control' }],
    },
    {
      emoji: '✍️', tag: 'Signing Password',
      title: 'Sign contracts\nlike a pro.',
      body: 'Create a signing password you\'ll remember. Use it to authorise every contract you create or agree to.',
      stats: [{ value: 'Binding', label: 'Instantly' }, { value: 'Simple', label: 'No complexity' }],
    },
  ],
}
...
function FeaturePanel({ step, role }) {
  const panels = role === ROLES.CLIENT ? PANELS.client : PANELS.freelancer
  const p = panels[Math.max(0, Math.min(step - 1, panels.length - 1))]
    ...
    <div className="hidden lg:flex flex-col justify-between w-[420px] flex-shrink-0 bg-[#0a0a0a] p-14">
      ...
      <div className="space-y-6">
        ...
        <h2 className="text-3xl font-extrabold text-white leading-snug">
          Trust between<br/><span className="text-green-500">every deal.</span>
        </h2>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            { v: 'Colle AI', l: 'Contract drafting' },
            { v: 'Trust', l: 'Score system' },
            { v: 'Versioned', l: 'Contracts' },
          ].map(item => (
    ...
    <div className="hidden lg:flex flex-col justify-between w-[420px] flex-shrink-0 bg-[#0a0a0a] p-14">
      ...
      <div className="space-y-5">
        ...
        <div className="space-y-2">
          {['✅ Account created & saved', '✅ Profile written to database', '✅ Signing password set', '✅ Trust score: 100/100', '🚀 Dashboard waiting for you'].map(t => (
            <p key={t} className="text-sm text-[#666] font-medium">{t}</p>
          ))}
        </div>
      </div>
    ...
    <div className="hidden lg:flex flex-col justify-between w-[420px] flex-shrink-0 bg-[#0a0a0a] p-14">
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <h2 className="text-3xl font-extrabold text-white leading-snug mt-2 whitespace-pre-line">{p.title}</h2>
        </div>
...
export default function Onboarding() {
  ...
  const [form, setForm] = useState({
    ...
    signing_password: '', signing_password_confirm: '',
  })
  ...
  const canProceed = () => {
    ...
    if (step === 4) return (
      form.signing_password.length >= 4 &&
      form.signing_password === form.signing_password_confirm
    )
    return true
  }
    ...
    if (step === 4) {
      ...
      try {
        await signUp({
          ...
          signing_password: form.signing_password,
          trust_score: 100,
        })
        setStep(5)
      } catch (err) {
  ...
  return (
    <div className={`min-h-screen flex transition-colors duration-300 ${c.bg}`}>
      <FeaturePanel step={step} role={role}/>
            ...
            <div className="flex items-center gap-2 mx-auto lg:mx-0">
              {STEPS.slice(0,5).map((s, i) => (
                <div key={s.id} className="flex items-center gap-2">
                  ...
                  {i < 4 && <div className={`w-5 h-px ${i < step ? 'bg-green-500' : c.border}`}/>}
                </div>
              ))}
            </div>
          )}
        ...
        <div className="flex-1 flex flex-col justify-center px-8 py-10 w-full max-w-md mx-auto">
          ...
          {step === 0 && (
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                {[
                  { r: ROLES.FREELANCER, icon: <User size={24}/>, title: "I'm a Freelancer", desc: 'Find jobs, sign contracts, get paid safely every time.', perks: ['Protected payments', 'AI contract drafting', 'Trust score'] },
                  { r: ROLES.CLIENT,     icon: <Briefcase size={24}/>, title: "I'm a Client",    desc: 'Post jobs, hire talent, pay only when work is done.', perks: ['Escrow protection', 'Milestone control', 'Verified talent'] },
                ].map(opt => (
                  <button key={opt.r} onClick={() => setRole(opt.r)}
                    className={`w-full flex items-start gap-4 p-5 rounded-2xl border-2 text-left transition-all
                      ${role === opt.r ? 'border-green-500 bg-green-500/5' : `${c.border} ${isDark ? 'hover:border-[#555]' : 'hover:border-[#999]'}`}`}>
          ...
          {step === 1 && (
            <div className="space-y-5">
              ...
              {[
                { label: 'Full Name', field: 'full_name', type: 'text', placeholder: 'Ade Okonkwo' },
                { label: 'Email Address', field: 'email', type: 'email', placeholder: 'you@example.com' },
              ].map(f => (
                <div key={f.field}>
                  <label className={`block text-xs font-bold uppercase tracking-widest ${c.muted} mb-2`}>{f.label}</label>
            ...
            <div className="space-y-5">
              <div>
                ...
                <p className={`text-sm ${c.light}`}>
                  Hey {firstName} 👋 {role === ROLES.FREELANCER
                    ...
                    {SKILLS.map(skill => (
                      <button key={skill} onClick={() => toggleSkill(skill)}
                        className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all
                          ${form.skills.includes(skill) ? 'bg-green-500 text-white border-green-500' : `${c.border} ${c.light}`}`}>
                        {skill}
                      </button>
                    ))}
                  </div>
                  {form.skills.length > 0 && <p className={`text-xs ${c.muted}`}>{form.skills.length}/5 selected</p>}
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {CATEGORIES.map(cat => (
                        <button key={cat} onClick={() => set('category', cat)}
                          className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all
                            ${form.category === cat ? 'bg-green-500 text-white border-green-500' : `${c.border} ${c.light}`}`}>
                          {cat}
                        </button>
                      ))}
                    </div>
          ...
          {step === 3 && (
            <div className="space-y-6">
              <div>
                ...
                <p className={`text-sm ${c.light}`}>Every user has a built-in wallet. Here's how it works for you as a {role}.</p>
              </div>
              <div className={`${c.bgMid} border ${c.border} rounded-2xl p-5 space-y-4`}>
                ...
                <div className={`h-px ${isDark ? 'bg-[#2e2e2e]' : 'bg-[#e0e0e0]'}`}/>
                <div className="space-y-3">
                  {(role === ROLES.CLIENT ? [
                    { icon: '💳', title: 'Deposit funds', desc: 'Add money to your wallet anytime' },
                    { icon: '🔒', title: 'Lock per contract', desc: 'Funds held safely in escrow' },
          ...
          {/* STEP 4 — Signing Password */}
          {step === 4 && (
            <div className="space-y-5">
              <div>
                <p className="text-xs font-bold uppercase tracking-widest text-green-500 mb-2">Step 5 of 5</p>
                <h1 className={`text-3xl font-extrabold tracking-tight ${c.text} mb-1`}>Create your signing password</h1>
                <p className={`text-sm ${c.light}`}>
                  This is separate from your login password. You'll enter it every time you sign a contract — make it something you'll remember.
                </p>
              </div>

              <div className={`p-4 rounded-xl bg-green-500/5 border border-green-500/20 space-y-1`}>
                {[
                  '✍️  Used only when signing contracts',
                  '🔒  Never used to log in',
                  '🧠  Make it memorable — you cannot recover it',
                  '✅  Minimum 4 characters',
                ].map(t => <p key={t} className="text-xs text-green-600 font-medium">{t}</p>)}
              </div>

              <div>
                <label className={`block text-xs font-bold uppercase tracking-widest ${c.muted} mb-2`}>Signing Password</label>
                <div className="relative">
                  <input type={showSign ? 'text' : 'password'} placeholder="Create a signing password"
                    value={form.signing_password} onChange={e => set('signing_password', e.target.value)}
                    className={`w-full pl-4 pr-12 py-3.5 rounded-xl border text-sm outline-none transition-all ${c.input}`}/>
                  <button type="button" onClick={() => setShowSign(!showSign)}
                    className={`absolute right-4 top-1/2 -translate-y-1/2 ${c.muted}`}>
                    {showSign ? <EyeOff size={15}/> : <Eye size={15}/>}
                  </button>
                </div>
              </div>
              <div>
                <label className={`block text-xs font-bold uppercase tracking-widest ${c.muted} mb-2`}>Confirm Signing Password</label>
                <div className="relative">
                  <input type={showSignConfirm ? 'text' : 'password'} placeholder="Confirm your signing password"
                    value={form.signing_password_confirm} onChange={e => set('signing_password_confirm', e.target.value)}
                    className={`w-full pl-4 pr-12 py-3.5 rounded-xl border text-sm outline-none transition-all
                      ${form.signing_password_confirm.length > 0 && form.signing_password_confirm !== form.signing_password
                        ? 'border-red-500/50 bg-red-500/5'
                        : c.input}`}/>
                  <button type="button" onClick={() => setShowSignConfirm(!showSignConfirm)}
                    className={`absolute right-4 top-1/2 -translate-y-1/2 ${c.muted}`}>
                    {showSignConfirm ? <EyeOff size={15}/> : <Eye size={15}/>}
                  </button>
                </div>
                {form.signing_password_confirm.length > 0 && form.signing_password_confirm !== form.signing_password && (
                  <p className="text-xs text-red-400 mt-1">Passwords do not match</p>
                )}
                {form.signing_password_confirm.length >= 4 && form.signing_password_confirm === form.signing_password && (
          ...
          {step === 5 && (
            <div className="text-center space-y-6">
              ...
              <div>
                <h1 className={`text-3xl font-extrabold tracking-tight ${c.text} mb-2`}>Welcome, {firstName}!</h1>
                <p className={`text-sm ${c.light}`}>Your account is live. Colle is ready to help you close your first deal.</p>
              </div>
              <div className={`${c.bgMid} border ${c.border} rounded-2xl p-5 text-left`}>
                {[
                  ['Name', form.full_name],
                  ...
                  <div key={label} className={`flex items-center justify-between py-3 border-b last:border-0 ${c.border}`}>
                    ...
                    <span className={`text-xs font-semibold ${c.text}`}>{val}</span>
                  </div>
                ))}
              </div>
